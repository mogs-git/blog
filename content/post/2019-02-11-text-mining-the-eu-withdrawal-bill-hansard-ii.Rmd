...
Now we have a table containing the **raw counts** every bigram in the Hansard. Take a peak and see which occur most. "Noble Lord" is a classic way of introducing another Lord in a speech in the House of Lords. Article 50 is a nice piece of writing detailing that the UK is going to leave the EU (sadface). Devolved administrations is another UK specific piece of lingo, meaning the governmental institutions of Wales, Scotland and Northern Ireland. Most of the other common bigrams are pretty self-explanatory. 

Now let's visualise the bigrams as a network, where each node is a word and the edges tell us which words are connected as bigrams and how often they were used (the strength of the line). 
...
```{r, echo=FALSE, fig.width=12, message=F, warning=F}
...
That's just a fancy way of saying we're going to label every word in a text as an "adjective" or a "noun" or a "verb" etc... Some people who are much smarter than I am have spent a good while <a href="https://opennlp.apache.org/" target="_blank">training machines</a> to do this and were so kind as to make it all open source. I'm not an expert, but from what I understand the OpenNLP tools are "pretty good" at accurately identifying the POS of each word, so that's what we're going to use. 
...
I've shown this bit of code to illustrate that beauty is in the eye of the beholder. The anno function is something I wrote, which was heftily borrowed from the <a href="https://www.rdocumentation.org/packages/openNLP/versions/0.2-6/topics/Maxent_POS_Tag_Annotator" target="_blank">openNLP documentation</a> (in R). All it does is take a bunch of text, and return a data table with each word and its associated 'POS tag'. 

There's quite a bit of cool stuff you can do with this data, and one thing people are fond of is counting the number of nouns, adjectives etc... and seeing what the most common ones are. We're going to skip all of that and ask a different question. Can we identify different groups of speakers based on the proportion of adjectives, nouns, and verbs they use? That's right, rather than pay attention to *which* words are being used, we're going to just count them (again). But the result is a bit surprising, and it's still quite involved. 
...
Our speakers each have data for 3 things: verbs, adjectives and nouns. This data can be arranged in 3-dimensional space like so.

![](/post/2019-02-11-text-mining-the-eu-withdrawal-bill-hansard-ii_files/euclidian1_5.png)

Then, the Euclidian distance is a simple way of saying "calculate the distance between each of these points".

![](/post/2019-02-11-text-mining-the-eu-withdrawal-bill-hansard-ii_files/euclidian2.png)

So we end up with a big table where we record the distance of each point from every other point. In fact, because you asked, here's a little snippet of it. 
...
This is what the "distance matrix" would look like for ten speakers.

Then we use a heirarchical clustering algorithm to try and find out how the points are organised into groups (or clusters) based on the distances between each point. There's a bit more to it than this, but a heirarchical clustering algorithm like `hclust()` in R looks through the distance matrix for the smallest number- indicating the two points that are closest together in 3D space in our image:
...
```{r}
members <- cutree(clust, k = 4)
members <- tibble(name=names(members), cluster=members)
```

You can imagine that each peer has now been assigned to one of three clusters based on their linguistic profile. 
