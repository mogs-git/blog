...
Then there are other forms of metadata. In the house of Lords, each speakers name is prefixed by a title- Lord, Baroness, Lady, etc... From this we can infer the gender of the MP. We can also extract their party (provided next to their title). If we wanted to, we could even search for external metadata such as age, or Alma Mater, but for now we will stick exclusively to what we can gleen from the Hansard itself. 

One can see, we are already building up the backbone of our analysis. To make things clearer, here is a diagram:

```{r pressure, echo=FALSE, fig.cap="Explanatory versus response variables", out.width = '100%'}
knitr::include_graphics("../eu_hansard/drawings/exp_vs_res.png")
```
...
Before we look at the text data itself, we need to get our bearings. We will begin by visualising who was present, and counting the number of speeches made by different groups of MPs. 
...
The figure shows that the majority of MPs belong to the three major parties or crossbench, and that the faction with the most MPs present are the conservatives. I have also coloured in the bars to indicate the proportion of male versus female MPs in each party. Needless to say, the vast majority of female representation in the house comes from Labour and the Liberal Democrats.    
...
However, even taking into account the disproportionate numbers of speakers present for each party, the Conservatives still speak more often than we would expect. This in turn means that other parties spoke disproportionately less often than what would happen if speeches were perfectly fairly distributed among each group present. To summarise, the Conservatives had more Lords, and still made more than the their fair share of speeches.

**Example calculation:** 
*There were 27 Conservative MPs and 93 MPs in total, `27 / 93 = 0.29` so 29% of the speakers present are Conservatives. 
...
In the next part, we will dig into the actual text of the Hansard. 
