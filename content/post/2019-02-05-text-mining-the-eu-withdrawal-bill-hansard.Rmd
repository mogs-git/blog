---
title: Text Mining the EU withdrawal bill Hansard (Part 2)
author: Matt Chaib
date: '2019-02-05'
...
```{r, echo = FALSE, message=FALSE, warning=FALSE, fig.cap="The relationship between the number of speeches a lord gives and the number of words they use."}
...
```{r, echo = FALSE, message=FALSE, warning=FALSE, fig.cap="As above but with specific boxplots for each peer."}
at_words %>%
  group_by(speech_id) %>%
  summarise(total = sum(n)) %>%
...
```{r, fig.width=12, fig.cap="The position of words on the y axis is irrelevant! Cheers to Kamil Slowikowski (https://slowkow.com/) for making ggrepel which organises the text so nicely."}
library(ggrepel)
